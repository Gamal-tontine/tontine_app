...
#         today = localdate()
#         if self.frequence == 'jour':
#             paid_members = self.members.filter(acquitement__date__date=today, acquitement__tontine=self)
#         elif self.frequence == 'semaine':
#             start_week = today - timedelta(days=today.weekday())
#             end_week = start_week + timedelta(days=6)
#             paid_members = self.members.filter(acquitement__date__range=(start_week, end_week), acquitement__tontine=self)
#         elif self.frequence == 'mois':
#             start_of_month = today.replace(day=1)
#             last_day_of_month = calendar.monthrange(today.year, today.month)[1]
#             end_of_month = today.replace(day=last_day_of_month)
#             paid_members = self.members.filter(acquitement__date__range=(start_of_month, end_of_month), acquitement__tontine=self)
#         else:
...
#     @property
#     def non_payers(self):
#         paid_members = Acquitement.objects.filter(
#             tontine=self,
#             user=OuterRef('pk')
#         )

#         return self.members.annotate(
#             has_paid=Exists(paid_members)
...
#         # Nombre de périodes payées par l'utilisateur
#         paid_periods = Acquitement.objects.filter(tontine=self, user=user).count()
#         missed_periods = periods_due - paid_periods
...
#     def members_due_amounts(self):
#         """
#         Retourne un dictionnaire contenant la dette restante de chaque membre,
#         optimisé pour réduire le nombre de requêtes en base de données.
#         """
...
#         # Vérifier si self.create_at est un datetime et extraire la date
#         created_at_date = self.create_at.date() if isinstance(self.create_at, datetime) else self.create_at

#         # Calculer le nombre de périodes dues
#         if self.frequence == 'jour':
#             periods_due = (today - created_at_date).days + 1
#         elif self.frequence == 'semaine':
#             periods_due = (today - created_at_date).days // 7 + 1
...
#         # Montant total dû par chaque membre
#         total_due_per_member = periods_due * self.amount
...
#         # Calculer la dette de chaque membre
#         for member in self.members.all():
#             total_paid = payments_dict.get(member.id, 0)
#             dues[member] = max(total_due_per_member - total_paid, 0)  # Évite les valeurs négatives

#         return dues  # Ex: {user1: 5000, user2: 20000}
